...
def find_cylinder_pairs(cylinders, reference_cylinders, max_radius):
    ...
    for i in xrange(len(cylinders)):
        ...
        for j in xrange(len(reference_cylinders)):
            distance = sqrt( (cylinders[i][0]-reference_cylinders[j][0])**2 + (cylinders[i][1]-reference_cylinders[j][1])**2  ) 
            if distance<minDis:
                minDis=distance
                association = (i,j)
        if minDis<max_radius:
            cylinder_pairs.append(association)
